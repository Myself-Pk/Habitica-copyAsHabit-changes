...
export default {
  ...
  mounted () {
    this.$root.$on('habitica::copy-as-habit', message => {
      alert('c1');
      ...
      // alert('c2');
      this.$root.$emit('bv::show::modal', 'copyAsHabit');
      // alert('c3');
    });
  },
  beforeDestroy () {
    this.$root.$off('habitica::copy-as-habit');
  },
  methods: {
    ...mapActions({
    ...
    close () {
      this.$root.$emit('bv::hide::modal', 'copyAsHabit');
    },
    saveHabit () {
